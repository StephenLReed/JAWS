...
public abstract class ReferenceSynset extends AbstractSynset
{
	 ...
	 */
	public int getWordIndex(String wordForm)
	{
		int index = -1;
	...
	{
		Synset[] synsets = (getReferences(type));
		if (synsets == null)
		{
	...
	{
		Synset[] synsets = null;
		SynsetPointer[] pointers1 = getPointers(type);
		if (pointers1 != null)
		{
			SynsetFactory factory = SynsetFactory.getInstance();
			synsets = new Synset[pointers1.length];
			for (int i = 0; i < synsets.length; i++)
			{
				synsets[i] = factory.getSynset(pointers1[i]);
			}
			clearPointers(type);
		}
		return synsets;
	 ...
	 */
	public SenseKey getSenseKey(int wordIndex)
	{
